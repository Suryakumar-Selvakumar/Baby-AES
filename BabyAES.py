#BabyAES.py

Sbox = (
    ,
)

InvSbox = (
    ,
)


# learnt from http://cs.ucsb.edu/~koc/cs178/projects/JT/aes.c
xtime = lambda a: (((a << 1) ^ 0x1B) & 0xFF) if (a & 0x80) else (a << 1)


Rcon = (
    0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40,
    0x80, 0x1B, 0x36, 0x6C, 0xD8, 0xAB, 0x4D, 0x9A,
    0x2F, 0x5E, 0xBC, 0x63, 0xC6, 0x97, 0x35, 0x6A,
    0xD4, 0xB3, 0x7D, 0xFA, 0xEF, 0xC5, 0x91, 0x39,
)


def text2matrix(text):
    matrix = []
    for i in range(4):
        matrix.append(list(text[i*4:(i+1)*4]))
    return matrix


def matrix2text(matrix):
    text = b''
    for i in range(4):
        for j in range(4):
            text += bytes([matrix[i][j]])
    return text


class AES:
    def __init__(self, master_key):
        self.change_key(master_key)

    def change_key(self, master_key):
        self.round_keys = text2matrix(master_key)

        for i in range(4, 4 * 11):
            self.round_keys.append([])
            if i % 4 == 0:
                byte = self.round_keys[i - 4][0] \
                     ^ Sbox[self.round_keys[i - 1][1]] \
                     ^ Rcon[i // 4]
                self.round_keys[i].append(byte)

                for j in range(1, 4):
                    byte = self.round_keys[i - 4][j] \
                         ^ Sbox[self.round_keys[i - 1][(j + 1) % 4]]
                    self.round_keys[i].append(byte)
            else:
                for j in range(4):
                    byte = self.round_keys[i - 4][j] \
                         ^ self.round_keys[i - 1][j]
                    self.round_keys[i].append(byte)

    def add_round_key(self, s, k):
        for i in range(4):
            for j in range(4):
                s[i][j] ^= k[i][j]
    
    def encrypt(self, plaintext):
        steps = []
        self.plain_state = text2matrix(plaintext)
        steps.append(('Initial State', matrix2text(self.plain_state)))

        self.add_round_key(self.plain_state, self.round_keys[:4])
        steps.append(('After AddRoundKey', matrix2text(self.plain_state)))

        for i in range(1, 10):
            self.round_encrypt(self.plain_state, self.round_keys[4 * i : 4 * (i + 1)])
            steps.append((f'After Round {i}', matrix2text(self.plain_state)))

        self.sub_bytes(self.plain_state)
        steps.append(('After SubBytes', matrix2text(self.plain_state)))

        self.shift_rows(self.plain_state)
        steps.append(('After ShiftRows', matrix2text(self.plain_state)))

        self.add_round_key(self.plain_state, self.round_keys[40:])
        steps.append(('After Final AddRoundKey', matrix2text(self.plain_state)))

        return steps

    def decrypt(self, ciphertext):
        steps = []
        self.cipher_state = text2matrix(ciphertext)
        steps.append(('Initial State', matrix2text(self.cipher_state)))

        self.add_round_key(self.cipher_state, self.round_keys[40:])
        steps.append(('After Initial AddRoundKey', matrix2text(self.cipher_state)))

        self.inv_shift_rows(self.cipher_state)
        steps.append(('After InvShiftRows', matrix2text(self.cipher_state)))

        self.inv_sub_bytes(self.cipher_state)
        steps.append(('After InvSubBytes', matrix2text(self.cipher_state)))

        for i in range(9, 0, -1):
            self.round_decrypt(self.cipher_state, self.round_keys[4 * i : 4 * (i + 1)])
            steps.append((f'After Round {10 - i}', matrix2text(self.cipher_state)))

        self.add_round_key(self.cipher_state, self.round_keys[:4])
        steps.append(('After Final AddRoundKey', matrix2text(self.cipher_state)))

        return steps


    def round_encrypt(self, state_matrix, key_matrix):
        self.sub_bytes(state_matrix)
        self.shift_rows(state_matrix)
        self.mix_columns(state_matrix)
        self.add_round_key(state_matrix, key_matrix)

    def round_decrypt(self, state_matrix, key_matrix):
        self.add_round_key(state_matrix, key_matrix)
        self.inv_mix_columns(state_matrix)
        self.inv_shift_rows(state_matrix)
        self.inv_sub_bytes(state_matrix)

    def sub_bytes(self, s):
        for i in range(4):
            for j in range(4):
                s[i][j] = Sbox[s[i][j]]


    def inv_sub_bytes(self, s):
        for i in range(4):
            for j in range(4):
                s[i][j] = InvSbox[s[i][j]]


    def shift_rows(self, s):
        s[0][1], s[1][1], s[2][1], s[3][1] = s[1][1], s[2][1], s[3][1], s[0][1]
        s[0][2], s[1][2], s[2][2], s[3][2] = s[2][2], s[3][2], s[0][2], s[1][2]
        s[0][3], s[1][3], s[2][3], s[3][3] = s[3][3], s[0][3], s[1][3], s[2][3]


    def inv_shift_rows(self, s):
        s[0][1], s[1][1], s[2][1], s[3][1] = s[3][1], s[0][1], s[1][1], s[2][1]
        s[0][2], s[1][2], s[2][2], s[3][2] = s[2][2], s[3][2], s[0][2], s[1][2]
        s[0][3], s[1][3], s[2][3], s[3][3] = s[1][3], s[2][3], s[3][3], s[0][3]

    def mix_single_column(self, a):
        # please see Sec 4.1.2 in The Design of Rijndael
        t = a[0] ^ a[1] ^ a[2] ^ a[3]
        u = a[0]
        a[0] ^= t ^ xtime(a[0] ^ a[1])
        a[1] ^= t ^ xtime(a[1] ^ a[2])
        a[2] ^= t ^ xtime(a[2] ^ a[3])
        a[3] ^= t ^ xtime(a[3] ^ u)


    def mix_columns(self, s):
        for i in range(4):
            self.mix_single_column(s[i])


    def inv_mix_columns(self, s):
        # see Sec 4.1.3 in The Design of Rijndael
        for i in range(4):
            u = xtime(xtime(s[i][0] ^ s[i][2]))
            v = xtime(xtime(s[i][1] ^ s[i][3]))
            s[i][0] ^= u
            s[i][1] ^= v
            s[i][2] ^= u
            s[i][3] ^= v

        self.mix_columns(s)
    
def hex_to_bytes(hex_string):
    return bytes.fromhex(hex_string)

aes_instance = AES(master_key=hex_to_bytes('2b7e151628aed2a6abf7158809cf4f3c'))